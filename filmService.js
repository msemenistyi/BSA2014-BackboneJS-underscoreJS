...
var FilmService = function() {
    ...
    this.getFilmList = function(id) {
        if (id) {
            ...
            if (item) {
                return _.pick(item, 'id', 'name', 'year');
            }
        } else {
    ...
    this.addFilm = function(item) {
        ...
        item['id'] = ids.length ? (_.max(ids) + 1) : 1;
        this.FilmList.push(film);

        return film;
    };

    this.updateFilm = function(id, attributes) {
        var film = this.getFilm(id);
        for (var attributeName in attributes) {
            if (film.hasOwnProperty(attributeName)) {
                if (film[attributeName] != attributes[attributeName]) {
                    film[attributeName] = attributes[attributeName];
                }
            }
        }
        this.FilmList[Number(id)] = film;

        return _.pick(film, 'id', 'name', 'year');
    };
